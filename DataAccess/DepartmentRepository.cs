...
public class DepartmentRepository
{
	...
	{
        return await _connection.ExecuteAsync("usp_UpdateDepartmentName", new { DNumber, DName }, commandType: CommandType.StoredProcedure) > -1;
    }

    public async Task<bool> UpdateDepartmentManager(int DNumber, int MgrSSN)
	{
        return await _connection.ExecuteAsync("usp_UpdateDepartmentManager", new { DNumber, MgrSSN }, commandType: CommandType.StoredProcedure) > -1;
    }


    public async Task<bool> DeleteDepartment(int DNumber)
	{
        return await _connection.ExecuteAsync("usp_DeleteDepartment", new { DNumber }, commandType: CommandType.StoredProcedure) > -1;
	}
